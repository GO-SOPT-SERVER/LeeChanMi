...
public class S3Service {
    ...
    public List<String> uploadImages(List<MultipartFile> multipartFileList, String folder) {
        ...
        for(MultipartFile file : multipartFileList){
            filePathList.add(uploadImage(file, folder));
        }
    ...
    public void deleteFile(String imageUrl) {
        ...
        boolean isObjectExist = amazonS3.doesObjectExist(bucket, imageKey);

        if (isObjectExist)
            amazonS3.deleteObject(bucket, imageKey);
        else
            throw new BadRequestException(Error.NOT_FOUND_FILE_EXCEPTION, Error.NOT_FOUND_FILE_EXCEPTION.getMessage());
    }
